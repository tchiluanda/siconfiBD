...
-   Entrada moderada: formado por 702 municípios. [Ituporanga-SC](https://cidades.ibge.gov.br/brasil/sc/ituporanga/panorama) é o município que ocupa a posição central do agrupamento. Seu percentual de atendimentos de paciente de outros municípios é 45% e percentual de saída de pacientes é 29,1%.
...
-   Saída moderada: para esses casos encontramos 1051 municípios. Aqui temos como ponto central a cidade de [São Luiz-RR](https://cidades.ibge.gov.br/brasil/rr/sao-luiz/panorama) com saída de 65,8% de paciente e entrada de 6,7%
...
O proximo passo foi identificar como os percentuais de gastos em hospitais se associam a cada um dos grupos[^3]. A figura logo abaixo nos permite identificar algumas conclusões.

[^3]: Tal como os dados sobre gastos informados logo na primeira seção do texto, aqui também utilizamos as informações vindas do SICONFI. A diferença é que para esse caso usamos uma [biblioteca em R](https://github.com/tchiluanda/siconfiBD) desenvolvida pela STN que facilita o consumo automatizado dos dados anuais de municípios.
...
É preciso dizer, porém, que há motivações para que esse tipo de desenho ocorra. Talvez não necessariamente de forma tão acentuada. A organização do SUS prevê regiões de saúde formadas por agrupamentos de municípios. Esse tipo de arranjo tem entre outros objetivos a busca da racionalização dos gastos, otimização de recursos e eficiência na rede de atenção[^4]. Essa caracterização do SUS prevê fluxos de pacientes e pode influenciar a decisão alocativa de orçamento municipal. O nosso trabalho não tem a intenção de mostrar as redes de causalidade que geram o quadro aqui apresentado. Acreditamos que essa é uma tarefa para equipes mais especializadas sobre o tema de saúde pública. Em todo caso deixamos aqui nossas análises, com códigos e dados abertos [^5], de forma que possam ser úteis para futuros trabalhos onde outros aspectos do atendimento hospitalar sejam investigados

[^4]: Para conhecer mais sobre as diretrizes gerais sobre regiões de saúde, leia a [resolução Nº 1, de 29 de setembro de 2011](https://bvsms.saude.gov.br/bvs/saudelegis/cit/2011/res0001_29_09_2011.html) do Ministério da Sáude.

[^5]: Os códigos utilizados, incluindo os processos de captura dos dados das fontes primárias estão disponíveis no [github](https://github.com/tchiluanda/siconfi_atendimento_hospitalar).

### Um pouco sobre o painel

Provavelmente você estará lendo esse texto a partir de um painel produzido pela Secretaria do Tesouro Nacional. Além desse texto o painel conta com páginas que de uma certa forma permitem localizar os municípios dentro dos quadros analíticos explorados ao longo do texto. Convidamos o leitor para fazer a navegação por entre as abas disponibilizadas, fazer filtros, baixar dados e capturar gráficos para outras ressignficações dos dados sobre atendimentos hospitares no Brasil e suas relações com execuções orçamentárias municipais.
