...
#' @examples get_perc_function_exp_municipality(year=2019, municipality = "3550308", gov_function = "saúde", expense_stage="Despesas Pagas"  )
#' @export


get_perc_function_exp_municipality<- function(year = 2019,municipality = NULL, gov_function = NULL, expense_stage=NULL ) {

  query_total<- paste0("with query_total as  (select a.ano, a.id_municipio, a.",pkg.env$coluna_estagio,", sum(a.valor) as total FROM `",pkg.env$database, ".",pkg.env$tabela_funcao, "` a where 1=1 ")

  if (!is.null(municipality)) {
    query_total <- paste0(query_total, " and cast(id_municipio as string) in (", str_c("'",municipality,"'", collapse = "," ),")")
  }
  ...
  if (!is.null(expense_stage)) {
    query_total <- paste0(query_total, " and lower(",pkg.env$coluna_estagio,") in (",str_to_lower( str_c("'",expense_stage, "'", collapse = "," )),")")
  }
  ...
  query<- paste0(" select df.*, (df.valor / qt.total)*100 as perc from  `",pkg.env$database, ".",pkg.env$tabela_funcao, "` df inner join query_total qt on df.ano = qt.ano and df.id_municipio = qt.id_municipio and df.",pkg.env$coluna_estagio, " = qt.",pkg.env$coluna_estagio, "  where 1=1 ")

  if (!is.null(municipality)) {
    query <- paste0(query, " and cast(df.id_municipio as string) in (", str_c("'",municipality,"'", collapse = "," ),")")
  }
  ...
  if (!is.null(expense_stage)) {
    query <- paste0(query, " and lower(df.",pkg.env$coluna_estagio,") in (",str_to_lower( str_c("'",expense_stage, "'", collapse = "," )),")")
  }
